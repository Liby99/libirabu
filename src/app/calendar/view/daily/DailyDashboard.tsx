...
import { hhmm } from "../../util/deadlineFormat";
...
function useTodoIndex() {
  ...
  useEffect(() => {
    ...
    // `calendar:changed` = broad (AI writes, daily-note saves); `calendar:todos-changed` = an event
    // note was patched in the drawer (base or per-occurrence) — refresh so its checkboxes show up.
    window.addEventListener("calendar:changed", onChanged);
    window.addEventListener("calendar:todos-changed", onChanged);
    return () => {
      window.removeEventListener("calendar:changed", onChanged);
      window.removeEventListener("calendar:todos-changed", onChanged);
    };
  }, [reload]);
