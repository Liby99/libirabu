...
import { firstDOW, weekStartDOM, weeksInMonth, resolveDate, MONTH_NAMES, WD, WD3 } from "../util/dates";
import { TRACKS, daysInMonth, YEAR, setCalendarYear } from "../model/api/mock";
...
function buildToday(items: Item[], z: number, focus: number, week: number, vp: Vp, scrollY: number, now: number, tlScroll: number, mul = 1, keyTag = "") {
  ...
  const pushNowLabel = (key: string, x: number, colW: number, lineY: number, active: boolean, gate = 1) => {
    const W = 84, GAP = 10, H = 30; // "Current Time" label box: width, gap from the line, height
...
function buildHover(items: Item[], z: number, focus: number, week: number, vp: Vp, scrollY: number, hover: Hover, tlScroll: number) {
  ...
  // ── Week: hovered day column (soft) + hour cell within it (strong) ──
  {
    ...
    const TW = 44, GAP = 10, TH = 20; // cursor time-tag box: width, gap from the line, height
    items.push({
