// All-day band-event store: optimistic local state + debounced PATCH + undo/redo integration.

import { useCallback, useEffect, useRef, useState } from "react";
import { BandEvent } from "../types/bandEventTypes";
import { apiToBand, fetchEvents, createBand, patchBand, deleteEvent } from "../api/apiClient";
...
export function useBandEvents(year: number, history: History, includeHidden = false) {
  ...
  const scheduleFlush = useCallback((id: string) => {
    ...
    timers.current.set(id, setTimeout(() => {
      ...
      if (ev) patchBand(ev).then(() => window.dispatchEvent(new Event("calendar:todos-changed"))).catch((e) => console.error("[calendar] patch band", e));
    }, PATCH_DEBOUNCE));
  }, []);
