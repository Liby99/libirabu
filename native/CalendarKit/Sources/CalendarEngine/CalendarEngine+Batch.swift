...
extension CalendarEngine {
    ...
    private func allBands(_ s: [String]) -> Bool { s.allSatisfy { kind(of: $0) == .band } }
    private func allTimeline(_ s: [String]) -> Bool { s.allSatisfy { kind(of: $0) == .timed || kind(of: $0) == .deadline } }
    private func allTimed(_ s: [String]) -> Bool { s.allSatisfy { kind(of: $0) == .timed } }
    ...
    public func batchMove(dx: Int, dy: Int) {
        ...
        let set = Set(srcs)
        if allBands(srcs) {
            if dx != 0 { batchBandDay(set, dx) } else if dy != 0 { batchBandTrack(set, dy) }
        } else if allTimeline(srcs) {
            if dx != 0 { batchTimelineDay(set, dx) } else if dy != 0 { batchTimelineVert(set, dy) }
        }
        scrollToSelected()
    }
    ...
    private func batchBandDay(_ set: Set<String>, _ dx: Int) {   // stay within the month, all-or-nothing
        ...
        guard !bands.isEmpty, bands.allSatisfy({ $0.startDay + dx >= 1 && $0.endDay + dx <= daysInMonth($0.year, $0.month) }) else { return }
        beginTxn()
        for i in items.bands.indices where set.contains(items.bands[i].id) { items.bands[i].startDay += dx; items.bands[i].endDay += dx }
        commitTxn()
    }
    private func batchBandTrack(_ set: Set<String>, _ dy: Int) {   // lanes 0…3, all-or-nothing
        ...
        beginTxn()
        for i in items.bands.indices where set.contains(items.bands[i].id) { items.bands[i].track += dy }
        commitTxn()
    }
    private func batchTimelineDay(_ set: Set<String>, _ dx: Int) {   // ±1 day, NOT across the month, all-or-nothing
        ...
        for i in items.events.indices where set.contains(items.events[i].id) {
            ...
            if nd.1 != items.events[i].month { return }   // crossed the month → abort the whole batch
            ev.append((i, nd))
        }
        for i in items.deadlines.indices where set.contains(items.deadlines[i].id) {
            ...
            if nd.1 != items.deadlines[i].month { return }
            dl.append((i, nd))
        }
        beginTxn()
        for (i, nd) in ev { items.events[i].year = nd.0; items.events[i].month = nd.1; items.events[i].day = nd.2 }
        for (i, nd) in dl { items.deadlines[i].year = nd.0; items.deadlines[i].month = nd.1; items.deadlines[i].day = nd.2 }
        commitTxn()
    }
    private func batchTimelineVert(_ set: Set<String>, _ dy: Int) {   // ±15 min, ROLLS across days (few limits)
        ...
        for i in items.events.indices where set.contains(items.events[i].id) {
            ...
            while s < 0 { let nd = addDays(e.year, e.month, e.day, -1); (e.year, e.month, e.day) = nd; s += 24 }
            while s >= 24 { let nd = addDays(e.year, e.month, e.day, 1); (e.year, e.month, e.day) = nd; s -= 24 }
            e.startHour = s; e.endHour = s + dur
            items.events[i] = e
        }
        for i in items.deadlines.indices where set.contains(items.deadlines[i].id) {
            var d = items.deadlines[i]; var h = d.hour + step
            while h < 0 { let nd = addDays(d.year, d.month, d.day, -1); (d.year, d.month, d.day) = nd; h += 24 }
            while h >= 24 { let nd = addDays(d.year, d.month, d.day, 1); (d.year, d.month, d.day) = nd; h -= 24 }
            d.hour = h
            items.deadlines[i] = d
        }
        commitTxn()
    ...
    public func batchResize(dx: Int, dy: Int) {
        ...
        if allBands(srcs), dx != 0 {
            let bands = items.bands.filter { set.contains($0.id) }
            func ne(_ b: BandEvent) -> Int { max(b.startDay, min(daysInMonth(b.year, b.month), b.endDay + dx)) }
            guard !bands.isEmpty, bands.allSatisfy({ ne($0) != $0.endDay }) else { return }
            beginTxn()
            for i in items.bands.indices where set.contains(items.bands[i].id) { items.bands[i].endDay = ne(items.bands[i]) }
            commitTxn()
        } else if allTimed(srcs), dy != 0 {   // deadlines have no duration → require ALL-timed
            let evs = items.events.filter { set.contains($0.id) }
            let step = CGFloat(dy) * 0.25
            func ne(_ e: TimedEvent) -> CGFloat { max(e.startHour + 0.25, min(24, e.endHour + step)) }
            guard !evs.isEmpty, evs.allSatisfy({ ne($0) != $0.endHour }) else { return }
            beginTxn()
            for i in items.events.indices where set.contains(items.events[i].id) { items.events[i].endHour = ne(items.events[i]) }
            commitTxn()
        }
    }
    ...
    public struct BatchDeleteSummary: Equatable, Sendable {
        ...
        public var isEmpty: Bool { toDelete + toHide == 0 }
        ...
        public var note: String {
            ...
            if recurring > 0 {
                p.append("\(recurring) of \(recurring == 1 ? "these is a recurring series" : "these are recurring series") and will be removed in full, including every occurrence.")
            }
            if promoted > 0 {
                p.append("\(promoted) \(promoted == 1 ? "is a promoted event" : "are promoted events") and will be removed along with \(promoted == 1 ? "its" : "their") promotion.")
            }
            if toHide > 0 {
                p.append("\(toHide) imported \(toHide == 1 ? "event" : "events") cannot be deleted and will be hidden instead.")
            }
            if alreadyHidden > 0 {
                p.append("\(alreadyHidden) imported \(alreadyHidden == 1 ? "event is" : "events are") already hidden and will be skipped.")
            }
    ...
    public func batchDeleteSummary() -> BatchDeleteSummary {
        ...
        for src in Set(selectedIds.map { sourceId(of: $0) }) {
            if isImported(src) {
                if isUserHidden(src) { s.alreadyHidden += 1 } else { s.toHide += 1 }
            } else {
                s.toDelete += 1
                if repeatConfig(src) != nil { s.recurring += 1 }
                if promoteTrack(src) != nil { s.promoted += 1 }
            }
        }
        return s
    }
    ...
    public func performBatchDelete() {
        ...
        beginTxn()
        for s in srcs {
            if isImported(s) { if !isUserHidden(s) { hideImportedSeries(s) } }
            else {
    ...
    public func batchSetTitle(_ title: String) {
        ...
        beginTxn()
        for i in items.events.indices where set.contains(items.events[i].id) { items.events[i].title = title }
        for i in items.bands.indices where set.contains(items.bands[i].id) { items.bands[i].title = title }
        for i in items.deadlines.indices where set.contains(items.deadlines[i].id) { items.deadlines[i].title = title }
        scheduleCommit()
    }
}
