...
public extension CalendarEngine {
    ...
    var activeCalendar: CalendarMeta? { registry.meta(registry.activeId) }
    var activeCalendarName: String { activeCalendar?.name ?? "Main" }
    var allCalendars: [CalendarMeta] { registry.all }
    ...
    var canRemoveCalendar: Bool { registry.all.count > 1 }
    ...
    func applyRemoteCalendars(upserts: [CalendarMeta], deletes: [String]) {
        for m in upserts { registry.upsertRemote(m) }
