...
enum Zipper {
    ...
    static func write(_ files: [String: Data], to dest: URL) throws {
        let tmp = FileManager.default.temporaryDirectory.appendingPathComponent("mdc-\(UUID().uuidString)", isDirectory: true)
    ...
    static func read(_ src: URL) throws -> [String: Data] {
        ...
        guard let en = FileManager.default.enumerator(at: tmp, includingPropertiesForKeys: [.isRegularFileKey]) else { return out }
        for case let f as URL in en {
            ...
            if let data = try? Data(contentsOf: f) { out[rel] = data }
        }
        return out
    }
    ...
    private static func run(_ path: String, _ args: [String], cwd: URL?) throws -> String {
        ...
        guard p.terminationStatus == 0 else {
            throw NSError(domain: "Zipper", code: Int(p.terminationStatus),
                          userInfo: [NSLocalizedDescriptionKey: String(data: data, encoding: .utf8) ?? "archive tool failed"])
        }
