...
public final class DemoController {
    ...
    private func sceneTimedWeek() async {
        ...
        await drag(from: CGPoint(x: x, y: y0), to: CGPoint(x: x, y: y1), over: 1.2) { p in engine.demoPointerDrag(atView: p) }
    ...
    private func sceneBandYear() async {
        ...
        await drag(from: CGPoint(x: x0, y: y), to: CGPoint(x: x1, y: y), over: 1.2) { p in engine.demoPointerDrag(atView: p) }
    ...
    private func scenePinchZoom() async {
        ...
        for i in 0..<3 { await pinch(around: centers[i], over: 0.55, spreadOut: true); try? await pause(0.9) }
        try? await pause(0.7)
    ...
    private func pause(_ s: Double) async throws { try await Task.sleep(nanoseconds: UInt64(s * 1_000_000_000)) }
    ...
    private func signalReady() {
        ...
        try? "ready\n".write(toFile: (dir as NSString).appendingPathComponent("ready.txt"), atomically: true, encoding: .utf8)
    }
...
public struct DemoCursorOverlay: View {
    ...
    public init(demo: DemoController) { self.demo = demo }
